...
if [ "$current_dir" != "quality" ]; then
    echo "You are not in the 'quality' directory."
    echo "Please change to that folder and run this script with 'sh scripts/cloud_eval.sh'"
    exit 1
fi
...
# create the run
pfazure run create -f scripts/yaml/index_job.yaml --name $run_name --stream
# evaluate the run
